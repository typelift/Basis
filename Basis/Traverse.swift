...
public func map<K, A, B>(_ f : @escaping (A) -> B) -> (Map<K, A>) -> Map<K, B> {
	return { m in
		switch m.match() {
			case .empty:
				return empty()
...
public func mapWithKey<K, A, B>(_ f : @escaping (K) -> (A) -> B) -> (Map<K, A>) -> Map<K, B> {
	return { m in
		switch m.match() {
			case .empty:
				return empty()
...
public func foldr<K, A, B>(_ f : @escaping (A) -> (B) -> B) -> (B) -> (Map<K, A>) -> B {
	return  { z in { m in
		switch m.match() {
			case .empty:
				return z
...
public func foldl<K, A, B>(_ f: @escaping (B) -> (A) -> B) -> (B) -> (Map<K, A>) -> B {
	return  { z in { m in
		switch m.match() {
			case .empty:
				return z
...
public func foldrWithKey<K, A, B>(_ f : @escaping (K) -> (A) -> (B) -> B) -> (B) -> (Map<K, A>) -> B {
	return  { z in { m in
		switch m.match() {
			case .empty:
				return z
...
public func foldlWithKey<K, A, B>(_ f: @escaping (A) -> (K) -> (B) -> A) -> (A) -> (Map<K, B>) -> A {
	return  { z in { m in
		switch m.match() {
			case .empty:
				return z
...
public func elems<K, A>(_ m : Map<K, A>) -> [A] {
	return foldr(<<|)([])(m)
}

/// Returns an array of all keys in the map in ascending order.
public func keys<K, A>(_ m : Map<K, A>) -> [K] {
