...
public func forkIO(io : IO<()>) -> IO<ThreadID> {
	return IO({ (let rw) in
		return (rw, BASERealWorld.forkWithStart({
...
public func getNumProcessors() -> IO<UInt> {
	return IO({ (let rw) in
		return (rw, BASERealWorld.CPUCount())
	})
}
...
public func killThread(tid : ThreadID) -> IO<()> {
	return IO({ (let rw) in
		return (rw, BASERealWorld.killThread(tid))
	})
}
...
public func yield() -> IO<()> {
	return IO({ (let rw) in
		return (rw, BASERealWorld.yieldThread())
	})
}
...
public func labelThread(name : String) -> IO<()> {
	return IO({ (let rw) in
		return (rw, name.withCString({ (let str) -> Void in
			return BASERealWorld.labelThreadWithName(str)
		}))
	})
}
